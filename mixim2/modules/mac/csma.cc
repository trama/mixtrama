...
void csma::initialize(int stage) {
	...
	if (stage == 0) {
		...
		EVT << "queueLength = " << queueLength
		<< " bitrate = " << bitrate
		<< " backoff method = " << par("backoffMethod").stringValue() << endl;

		EVT << "MAC address " << myMacAddr << "finished csma init stage 1." << endl;
	}
}
...
void csma::handleUpperMsg(cMessage *msg) {
	...
	EVT<<"CSMA received a message from upper layer, name is " << msg->getName() <<", CInfo removed, mac addr="<< cInfo->getNextHopMac()<<endl;
	...
	if(useMACAcks) {
		if(SeqNrParent.find(dest) == SeqNrParent.end()) {
			...
			EVT << "Adding a new parent to the map of Sequence numbers:" << dest << endl;
		}
		else {
			macPkt->setSequenceId(SeqNrParent[dest]);
			EVT << "Packet send with sequence number = " << SeqNrParent[dest] << endl;
			SeqNrParent[dest]++;
		}
	}
	...
	EVT <<"pkt encapsulated, length: " << macPkt->getBitLength() << "\n";
...
void csma::updateStatusIdle(t_mac_event event, cMessage *msg) {
	...
	case EV_SEND_REQUEST:
		if (macQueue.size() <= queueLength) {
			...
			EVT<<"(1) FSM State IDLE_1, EV_SEND_REQUEST and [TxBuff avail]: startTimerBackOff -> BACKOFF." << endl;
			...
			EVT << "(12) FSM State IDLE_1, EV_SEND_REQUEST and [TxBuff not avail]: dropping packet -> IDLE." << endl;
	...
	case EV_DUPLICATE_RECEIVED:
		EVT << "(15) FSM State IDLE_1, EV_DUPLICATE_RECEIVED: setting up radio tx -> WAITSIFS." << endl;
	...
	case EV_FRAME_RECEIVED:
		EVT << "(15) FSM State IDLE_1, EV_FRAME_RECEIVED: setting up radio tx -> WAITSIFS." << endl;
...
void csma::updateStatusBackoff(t_mac_event event, cMessage *msg) {
	...
	case EV_TIMER_BACKOFF:
		EVT<< "(2) FSM State BACKOFF, EV_TIMER_BACKOFF:"
	...
	case EV_DUPLICATE_RECEIVED:
		...
		EVT << "(28) FSM State BACKOFF, EV_DUPLICATE_RECEIVED:";
		if(useMACAcks) {
			EVT << "suspending current transmit tentative and transmitting ack";
	...
	case EV_FRAME_RECEIVED:
		...
		// transmit ack,
		// and resume transmission when entering manageQueue()
		EVT << "(28) FSM State BACKOFF, EV_FRAME_RECEIVED:";
		if(useMACAcks) {
			EVT << "suspending current transmit tentative and transmitting ack";
			transmissionAttemptInterruptedByRx = true;
			cancelEvent(backoffTimer);
			...
			EVT << "sending frame up and resuming normal operation.";
		}
		sendUp(decapsMsg(static_cast<MacPkt *>(msg)));
		delete msg;
		break;
	case EV_BROADCAST_RECEIVED:
		EVT << "(29) FSM State BACKOFF, EV_BROADCAST_RECEIVED:"
...
void csma::updateStatusCCA(t_mac_event event, cMessage *msg) {
	...
	case EV_TIMER_CCA:
	{
		EVT<< "(25) FSM State CCA_3, EV_TIMER_CCA" << endl;
		bool isIdle = phy->getChannelState().isIdle();
		if(isIdle) {
			EVT << "(3) FSM State CCA_3, EV_TIMER_CCA, [Channel Idle]: -> TRANSMITFRAME_4." << endl;
			...
			EVT << "(7) FSM State CCA_3, EV_TIMER_CCA, [Channel Busy]: "
			...
			if(NB> macMaxCSMABackoffs) {
				// drop the frame
				EVT << "Tried " << NB << " backoffs, all reported a busy "
	...
	case EV_DUPLICATE_RECEIVED:
		EVT << "(26) FSM State CCA_3, EV_DUPLICATE_RECEIVED:";
		if(useMACAcks) {
			EVT << " setting up radio tx -> WAITSIFS." << endl;
			// suspend current transmission attempt,
			// transmit ack,
			// and resume transmission when entering manageQueue()
			transmissionAttemptInterruptedByRx = true;
	...
	case EV_FRAME_RECEIVED:
		EVT << "(26) FSM State CCA_3, EV_FRAME_RECEIVED:";
		if(useMACAcks) {
			EVT << " setting up radio tx -> WAITSIFS." << endl;
			// suspend current transmission attempt,
			// transmit ack,
		...
		break;
	case EV_BROADCAST_RECEIVED:
		EVT << "(24) FSM State BACKOFF, EV_BROADCAST_RECEIVED:"
		<< " Nothing to do." << endl;
		sendUp(decapsMsg(static_cast<MacPkt *>(msg)));
		delete msg;
		break;
...
void csma::updateStatusTransmitFrame(t_mac_event event, cMessage *msg) {
	if (event == EV_FRAME_TRANSMITTED) {
		...
		if (packet->getDestAddr() != L2BROADCAST) {
			//unicast
			EVT << "(4) FSM State TRANSMITFRAME_4, "
			   << "EV_FRAME_TRANSMITTED [Unicast]: ";
		} else {
			//broadcast
			EVT << "(27) FSM State TRANSMITFRAME_4, EV_FRAME_TRANSMITTED "
			   << " [Broadcast]";
			expectAck = false;
		}

		if(expectAck) {
			EVT << "RadioSetupRx -> WAITACK." << endl;
			updateMacState(WAITACK_5);
			startTimer(TIMER_RX_ACK);
		} else {
			EVT << ": RadioSetupRx, manageQueue..." << endl;
			macQueue.pop_front();
			delete packet;
			manageQueue();
		}
...
void csma::updateStatusWaitAck(t_mac_event event, cMessage *msg) {
	...
	case EV_ACK_RECEIVED:
		EVT<< "(5) FSM State WAITACK_5, EV_ACK_RECEIVED: "
		...
		break;
	case EV_ACK_TIMEOUT:
		EVT << "(12) FSM State WAITACK_5, EV_ACK_TIMEOUT:"
	...
	case EV_FRAME_RECEIVED:
		sendUp(decapsMsg(static_cast<MacPkt*>(msg)));
	case EV_DUPLICATE_RECEIVED:
		EVT << "Error ! Received a frame during SIFS !" << endl;
		delete msg;
		break;
	default:
		fsmError(event, msg);
...
void csma::manageMissingAck(t_mac_event event, cMessage *msg) {
	if (txAttempts < macMaxFrameRetries + 1) {
		// increment counter
		txAttempts++;
		EVT<< "I will retransmit this packet (I already tried "
		<< txAttempts << " times)." << endl;
	} else {
		// drop packet
		EVT << "Packet was transmitted " << txAttempts
...
void csma::updateStatusSIFS(t_mac_event event, cMessage *msg) {
	...
	switch (event) {
	case EV_TIMER_SIFS:
		EVT<< "(17) FSM State WAITSIFS_6, EV_TIMER_SIFS:"
	...
	case EV_TIMER_BACKOFF:
		...
		// and stay here.
		EVT << "(16) FSM State WAITSIFS_6, EV_TIMER_BACKOFF. "
...
void csma::updateStatusTransmitAck(t_mac_event event, cMessage *msg) {
	...
	if (event == EV_FRAME_TRANSMITTED) {
		EVT<< "(19) FSM State TRANSMITACK_7, EV_FRAME_TRANSMITTED:"
		<< " ->manageQueue." << endl;
		phy->setRadioState(Radio::RX);
		//		delete msg;
		manageQueue();
...
void csma::updateStatusNotIdle(cMessage *msg) {
	EVT<< "(20) FSM State NOT IDLE, EV_SEND_REQUEST. Is a TxBuffer available ?" << endl;
	if (macQueue.size() <= queueLength) {
		macQueue.push_back(static_cast<MacPkt *>(msg));
		EVT << "(21) FSM State NOT IDLE, EV_SEND_REQUEST"
		<<" and [TxBuff avail]: enqueue packet and don't move." << endl;
	} else {
		// queue is full, message has to be deleted
		EVT << "(22) FSM State NOT IDLE, EV_SEND_REQUEST"
...
void csma::executeMac(t_mac_event event, cMessage *msg) {
	...
	if(macState != IDLE_1 && event == EV_SEND_REQUEST) {
		...
		case TRANSMITACK_7:
			...
			break;
		default:
			EVT << "Error in CSMA FSM: an unknown state has been reached. macState=" << macState << endl;
		}
	}
}
...
void csma::manageQueue() {
	if (macQueue.size() != 0) {
		EVT<< "(manageQueue) there are " << macQueue.size() << " packets to send, entering backoff wait state." << endl;
		...
		EVT << "(manageQueue) no packets to send, entering IDLE state." << endl;
		updateMacState(IDLE_1);
	}
}
...
void csma::fsmError(t_mac_event event, cMessage *msg) {
	EVT<< "FSM Error ! In state " << macState << ", received unknown event:" << event << "." << endl;
	if (msg != NULL)
	delete msg;
}
...
void csma::startTimer(t_mac_timer timer) {
	if (timer == TIMER_BACKOFF) {
		...
		EVT<< "(startTimer) ccaTimer value=" << ccaTime
		...
		assert(useMACAcks);
		EVT << "(startTimer) sifsTimer value=" << sifs << endl;
		scheduleAt(simTime()+sifs, sifsTimer);
	} else if (timer==TIMER_RX_ACK) {
		assert(useMACAcks);
		EVT << "(startTimer) rxAckTimer value=" << macAckWaitDuration << endl;
		scheduleAt(simTime()+macAckWaitDuration, rxAckTimer);
	} else {
		EVT << "Unknown timer requested to start:" << timer << endl;
	}
}
...
double csma::scheduleBackoff() {
	...
	case EXPONENTIAL:
	{
		...
		EVT<< "(startTimer) backoffTimer value=" << backoffTime
		<< " (BE=" << BE << ", 2^BE-1= " << v << "r="
		<< r << ")" << endl;
		break;
	}
	case LINEAR:
	{
		...
		EVT<< "(startTimer) backoffTimer value=" << backoffTime << endl;
		break;
	}
	case CONSTANT:
	{
...
void csma::handleSelfMsg(cMessage *msg) {
	...
	if(msg==backoffTimer)
		...
		EVT << "CSMA Error: unknown timer fired:" << msg << endl;
}
...
void csma::handleLowerMsg(cMessage *msg) {
	...
	EVT<< "Received frame name= " << macPkt->getName()
	...
	{
		if(!useMACAcks) {
			EVT << "Received a data packet addressed to me." << endl;
			nbRxFrames++;
			...
			if(strcmp(macPkt->getName(), "CSMA-Ack") != 0) {
				...
				EVT << "Received a data packet addressed to me,"
				...
				if(SeqNrChild.find(src) == SeqNrChild.end()) {
					...
					EVT << "Adding a new child to the map of Sequence numbers:" << src << endl;
					executeMac(EV_FRAME_RECEIVED, macPkt);
				}
				else {
					ExpectedNr = SeqNrChild[src];
					EVT << "Expected Sequence number is " << ExpectedNr <<
				...
				if(macPkt->getSrcAddr() == firstPacket->getDestAddr()) {
					...
					EVT << "Error! Received an ack from an unexpected source: src=" << macPkt->getSrcAddr() << ", I was expecting from node addr=" << firstPacket->getDestAddr() << endl;
					delete macPkt;
				}
			} else {
				EVT << "Error! Received an Ack while my send queue was empty. src=" << macPkt->getSrcAddr() << "." << endl;
				delete macPkt;
			}
		}
	}
...
void csma::handleLowerControl(cMessage *msg) {
	if (msg->getKind() == MacToPhyInterface::TX_OVER) {
		...
		EVT<< "control message: PACKED DROPPED" << endl;
	} else if (msg->getKind() == MacToPhyInterface::RADIO_SWITCHING_OVER) {
		EVT<< "control message: RADIO_SWITCHING_OVER" << endl;
	} else {
		EVT << "Invalid control message type (type=NOTHING) : name="
