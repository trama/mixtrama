...
void Mac80211::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        EVT << "SIFS: " << SIFS << " DIFS: " << DIFS << " EIFS: " << EIFS << endl;
    }
    else if(stage == 1) {
...
void Mac80211::handleUpperMsg(cMessage *msg)
{
    ...
    EVT << "Mac80211::handleUpperMsg " << msg->getName() << "\n";
    ...
    if(fromUpperLayer.size() == queueLength) {
        ...
        sendControlUp(msg);
        EVT << "packet " << msg << " received from higher layer but MAC queue is full, signalling error\n";
        return;
    }

    Mac80211Pkt *mac = encapsMsg(pkt);
    EVT << "packet " << pkt << " received from higher layer, dest=" << mac->getDestAddr() << ", encapsulated\n";
    ...
    {
        EVT << "enqueued, will be transmitted later\n";
    }
}
...
Mac80211Pkt *Mac80211::encapsMsg(cPacket * netw)
{
    ...
    EVT <<"CInfo removed, mac addr="<< cInfo->getNextHopMac()<<endl;
    ...
    EVT <<"pkt encapsulated, length: " << pkt->getBitLength() << "\n";
...
void Mac80211::handleLowerMsg(cMessage *msg)
{
    ...
    if(radioState == Radio::RX) {
        ...
        EVT << " handleLowerMsg frame " << af << " received\n";
        addNeighbor(af);
        if (contention->isScheduled()) {
        ...
        EVT << " handleLowerMsg frame " << af << " deleted, strange race condition\n";
        delete af;
    }
}
...
{
    EVT << simTime() << " handleLowerControl " << msg->getName() << "\n";
    switch(msg->getKind()) {
    ...
    case Decider80211::BITERROR:
    {
        ...
        if(radioState == Radio::RX) {
            ...
            EVT << " frame " << msg->getName() << " deleted, strange race condition\n";
            delete msg;
        }
        break;
    }
    case MacToPhyInterface::TX_OVER:
        EVT << "PHY indicated transmission over" << endl;
...
{
    EVT << simTime() << " handleSelfMsg " << msg->getName() << "\n";
    switch (msg->getKind())
    {
...
{
    EVT << "handle msg not for me " << af->getName() << "\n";
    ...
    if(duration != 0) {
        ...
        if (state == QUIET)
        {
            ...
            if (nav->getArrivalTime() < simTime() + duration)
            {
                ...
                EVT << "NAV timer started for: " << duration << " State QUIET\n";
            }
        }
        // other states
        else
        {
            ...
            scheduleAt(simTime() + duration, nav);
            EVT << "NAV timer started, not QUIET: " << duration << endl;
...
{
    EVT << "handle msg for me " << af->getName() << " in " <<  stateName(state) << "\n";
    ...
    case WFDATA:  // waiting for DATA
        if (af->getKind() == DATA) {
            ...
            EVT << "unexpected message -- probably a collision of RTSs\n";
            delete af;
        }
        break;
    ...
    case BUSY: // currently transmitting an ACK or a BROADCAST packet
        if(switching) {
            EVT << "message received during radio state switchover\n";
            delete af;
        }
        else {
...
void Mac80211::handleDATAframe(Mac80211Pkt * af)
{
    ...
    if(af->getRetry() && (it->fsc == af->getSequenceControl())) {
        EVT << "Mac80211::handleDATAframe suppressed duplicate message " << af
           << " fsc: " << it->fsc << "\n";
    }
    else {
...
{
    bool rtscts = true;
    EVT << simTime() << " handleTimeoutTimer " << stateName(state) << "\n";
    if(state == WFCTS) {
        longRetryCounter++;
        remainingBackoff = backoff();
    }
...
{
    EVT << "transmission of packet is over\n";
    if(state == BUSY) {
        if(nextIsBroadcast) {
            shortRetryCounter = 0;
            longRetryCounter = 0;
...
void Mac80211::sendDATAframe(Mac80211Pkt *af)
{
    ...
    EVT << "sending DATA  to " << frame->getDestAddr() << " with bitrate " << br << endl;
    // send DATA frame
    sendDown(frame);
...
void Mac80211::sendRTSframe()
{
    ...
    EVT << " Mac80211::sendRTSframe duration: " <<  packetDuration(LENGTH_RTS, br) << " br: " << br << "\n";
...
void Mac80211::sendCTSframe(Mac80211Pkt * af)
{
    ...
    EVT << " Mac80211::sendCTSframe duration: " <<  packetDuration(LENGTH_CTS, br) << " br: " << br << "\n";
    // send CTS frame
    sendDown(frame);
...
void Mac80211::beginNewCycle()
{
    ...
    if (nav->isScheduled()) {
        EVT << "cannot beginNewCycle until NAV expires at t " << nav->getArrivalTime() << endl;
        return;
    }
    ...
    if (!fromUpperLayer.empty()) {
        ...
        if(!contention->isScheduled()) {
            ...
            EVT << simTime() << " do contention: medium = " << channel.info() << ", backoff = "
...
simtime_t Mac80211::backoff(bool rtscts) {
    ...
    EVT << simTime() << " random backoff = " << value << endl;
...
void Mac80211::testMaxAttempts()
{
    if ((longRetryCounter >= LONG_RETRY_LIMIT) || (shortRetryCounter >= SHORT_RETRY_LIMIT)) {
        EVT << "retry limit reached src: "<< shortRetryCounter << " lrc: " << longRetryCounter << endl;
        // initialize counter
        longRetryCounter = 0;
        shortRetryCounter = 0;
...
void Mac80211::receiveBBItem(int category, const BBItem *details, int scopeModuleId)
{
    ...
    EVT << simTime() << " receiveBBItem " << details->info() << endl;
...
simtime_t Mac80211::timeOut(Mac80211MessageKinds type, double br)
{
    ...
    case RTS:
        ...
        EVT << " Mac80211::timeOut RTS " << time_out << "\n";
        break;
    case DATA:
        time_out = SIFS + packetDuration(fromUpperLayer.front()->getBitLength(), br) + packetDuration(LENGTH_ACK, br) + delta;
        EVT << " Mac80211::timeOut DATA " << time_out << "\n";
        break;
    default:
        EVT << "Unused frame type was given when calling timeOut(), this should not happen!\n";
    }
    return time_out;
}
...
{
    if (state==newState)
        EVT << "staying in state " << stateName(state) << "\n";
    else
        EVT << "state " << stateName(state) << " --> " << stateName(newState) << "\n";
    state = newState;
}
...
void Mac80211::suspendContention()  {
    ...
    //if(requestReturned || chSenseRequest->isScheduled()) {
        ...
        EVT << simTime() << " suspend contention: "
		   << "began " << chSenseStart
        ...
        if(quietTime < currentIFS) {
            EVT << "suspended during D/EIFS (no backoff)" << endl;
        }
        else {
            ...
            EVT << "backoff was " << ((contention->getSenseTimeout() - currentIFS))/ST
               << " slots, now " << remainingSlots << " slots remain" << endl;
        }

        EVT << "suspended backoff timer, remaining backoff time: "
           << remainingBackoff << endl;
...
void Mac80211::addNeighbor(Mac80211Pkt *af) {
    ...
    EVT << "updated information for neighbor: " << srcAddress
