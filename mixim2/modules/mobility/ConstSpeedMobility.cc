...
void ConstSpeedMobility::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        EVT << "Initialize: move speed: " << move.getSpeed() << " (" << par("speed").doubleValue() << ")"
           << " pos: " << move.info() << endl;
    }
    else if( stage == 1 ){
...
{
    EVT << "start setTargetPosistion: " << move.info() << endl;
	...
	EVT << "new targetPos: " << targetPos.info() << " distance=" << distance
    ...
    EVT << "stepSize: " << stepSize.info() << " target: " << (stepSize*numSteps).info() << endl;

    step = 0;
    move.setDirectionByTarget(targetPos);

    EVT << "end setTargetPosistion: " << move.info() << endl;
}
...
void ConstSpeedMobility::makeMove()
{
    ...
    if( step == numSteps ){
		...
		EVT << "stepping forward. step #=" << step
		   << " startPos: " << move.getStartPos().info() << endl;


		// get new target position
		EVT << "destination reached.\n"
		   << move.info() << endl;
		setTargetPosition();
    }
